...
@section('content')
    <div class="max-w-4xl space-y-8 anim-up" style="animation-delay: 0.1s">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-semibold tracking-tight text-white">New Module</h1>
                <p class="text-zinc-400 mt-1 font-medium text-sm">Course: {{ $course->title ?? 'Название курса' }}</p>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('admin.courses.modules.store', $course ?? 1) }}" class="bg-zinc-900 rounded-3xl border border-white/5 divide-y divide-white/5 shadow-xl shadow-black/20">
            @csrf
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full bg-zinc-950 border border-white/10 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 px-4 py-3 rounded-xl text-sm text-white outline-none transition-all duration-300 resize-none">{{ old('description') }}</textarea>
                </div>
            ...
            <div class="p-8 flex items-center gap-4 justify-end bg-zinc-950/30 rounded-b-3xl">
                ...
                <button type="submit" class="flex items-center gap-2 px-8 py-3 text-sm font-semibold text-black bg-white rounded-2xl hover:bg-white/90 transition-all active:scale-95 shadow-lg shadow-white/10">
                    <i data-lucide="check" class="w-4 h-4"></i> Создать модуль
                </button>
            </div>
        </form>
    </div>
