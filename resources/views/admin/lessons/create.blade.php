...
@section('content')
    <div class="max-w-4xl space-y-8 anim-up" style="animation-delay: 0.1s">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-semibold tracking-tight text-white">New Lesson</h1>
                <p class="text-zinc-400 mt-1 font-medium text-sm">Module: {{ $module->title ?? 'Название модуля' }}</p>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('admin.modules.lessons.store', $module ?? 1) }}" class="bg-zinc-900 rounded-3xl border border-white/5 divide-y divide-white/5 shadow-xl shadow-black/20">
            @csrf
            ...
            <div class="p-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select name="content_type" id="content_type" required class="appearance-none w-full bg-zinc-950 border border-white/10 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 px-4 py-3 pr-10 rounded-xl text-sm text-white outline-none transition-all duration-300 cursor-pointer">
                            <option value="text" {{ old('content_type') === 'text' ? 'selected' : '' }} class="bg-zinc-900">Text (Markdown)</option>
                            <option value="video" {{ old('content_type') === 'video' ? 'selected' : '' }} class="bg-zinc-900">Video</option>
                            <option value="interactive" {{ old('content_type') === 'interactive' ? 'selected' : '' }} class="bg-zinc-900">Interactive</option>
                        </select>
                ...
                <div id="video_url_field" class="{{ old('content_type') !== 'video' ? 'hidden' : '' }} space-y-2 anim-fade">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="content" rows="12" required
                              class="w-full bg-zinc-950 border border-white/10 focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 px-4 py-3 rounded-xl text-sm text-white outline-none transition-all duration-300 font-mono resize-y">{{ old('content') }}</textarea>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                    ...
                    <label class="flex items-center gap-4 p-4 rounded-2xl border border-white/5 bg-zinc-950/50 hover:bg-white/5 transition-colors cursor-pointer group">
                        <div class="relative flex items-center">
                            <input type="checkbox" name="is_free" value="1" {{ old('is_free') ? 'checked' : '' }} class="peer sr-only">
            ...
            <div class="p-8 flex items-center gap-4 justify-end bg-zinc-950/30 rounded-b-3xl">
                ...
                <button type="submit" class="flex items-center gap-2 px-8 py-3 text-sm font-semibold text-black bg-white rounded-2xl hover:bg-white/90 transition-all active:scale-95 shadow-lg shadow-white/10">
                    <i data-lucide="check" class="w-4 h-4"></i> Создать урок
                </button>
            </div>
        </form>
    </div>
